...
macro(cmakejson_run_macro_over_parsed_range _varprefix _macro)
    if(NOT COMMAND ${_macro})
        message(${CMakeJSON_MSG_ERROR_TYPE} "cmakejson_run_macro_over_parsed_range: called with invalid command '${_macro}'")
    endif()
    if(DEFINED ${_varprefix}_LENGTH AND ${_varprefix}_LENGTH GREATER_EQUAL 1)
        math(EXPR cmakejson_run_macro_id "${cmakejson_run_macro_id}+1") # required to avoid collision with recursive cmakejson_run_macro_over_parsed_range calls
        math(EXPR cmakejson_run_macro_over_parsed_range_${cmakejson_run_macro_id} "${${_varprefix}_LENGTH}-1")
        foreach(cmakejson_run_macro_over_parsed_range_index_${cmakejson_run_macro_id} RANGE 0 ${cmakejson_run_macro_over_parsed_range_${cmakejson_run_macro_id}})
            cmakejson_message_if(CMakeJSON_DEBUG_RANGE_LOOP "Index: ${cmakejson_run_macro_over_parsed_range_${cmakejson_run_macro_id}}/${cmakejson_run_macro_over_parsed_range_${cmakejson_run_macro_id}}")
            cmakejson_message_if(CMakeJSON_DEBUG_RANGE_LOOP "Command: ${_macro}(${_varprefix}_${cmakejson_run_macro_over_parsed_range_index_${cmakejson_run_macro_id}} ${ARGN})")
            cmake_language(CALL ${_macro} "${_varprefix}_${cmakejson_run_macro_over_parsed_range_index_${cmakejson_run_macro_id}}" ${ARGN})
        endforeach()
    endif()
    unset(cmakejson_run_macro_over_parsed_range_${cmakejson_run_macro_id})
    unset(cmakejson_run_macro_over_parsed_range_index_${cmakejson_run_macro_id})
    math(EXPR cmakejson_run_macro_id "${cmakejson_run_macro_id}-1")
    if(cmakejson_run_macro_id LESS_EQUAL 0)
        unset(cmakejson_run_macro_id)
    endif()
endmacro()

function(cmakejson_run_func_over_parsed_range _varprefix _function)
    if(NOT COMMAND ${_function})
        message(${CMakeJSON_MSG_ERROR_TYPE} "cmakejson_run_func_over_parsed_range: called with invalid command '${_function}'")
    endif()
    ...
    if(DEFINED ${_varprefix}_LENGTH AND ${_varprefix}_LENGTH GREATER_EQUAL 1)
        ...
        list(REMOVE_ITEM vars_after ${vars_before} vars_before)
        cmakejson_return_to_parent_scope(${vars_after})
        set(CMakeJSON_PARENT_SCOPE_VARS ${vars_after})
